...
#
# 현황(2026-07-30): warmup(몸풀기)·standing(서서 운동)·seated(근력 운동) 서빙 대상 → available=true. cooldown은 앱 번들 루틴.
#   - warmup: warmup.mp4, standing: standing_ex.mp4, seated: strength_ex.mp4 를
#     EC2(/opt/ah0404/media/videos)에 업로드, nginx /videos/ 로 서빙. 앱은 {BASE_URL}/videos/{파일명} 재생.
#   ⚠️ standing/seated 영상은 EC2 업로드 후 이 PR을 머지해야 한다(파일 없이 available=true면 404).
#   ※ 2026-07-30 신규 가공본으로 교체: 파일명을 standing_ex.mp4/strength_ex.mp4 로 변경(신규 URL)해 앱
#     영상캐시(URL 키잉, 200MB LRU)를 강제 무효화 → 기기가 옛 영상을 재생하지 않도록 한다. 업로드 완료.
