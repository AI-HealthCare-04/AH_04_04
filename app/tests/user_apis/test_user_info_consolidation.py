...
# 분산돼 있던 birth_date·성별·보유포인트를 한 응답으로 모으는 조립 로직을,
# fake 레포지토리를 주입해 DB 없이 검증한다(risk 서비스 테스트와 동일 방식).
# (운동강도 단계는 난이도 폐기(#428)로 응답에서 제거됨)
# =====================================================================================
...
from app.models.enums import AuthProvider, OnboardingStatus, Sex
...
# 건강프로필이 아직 없고 적립도 없으면: birth_date/sex=null, 포인트=0.
async def test_get_user_info_uses_safe_defaults_when_sources_missing() -> None:
    service = _service_with_fakes(profile=None, current_points=0)
...
def test_user_info_response_shape_includes_consolidated_fields() -> None:
    ...
    assert {"birth_date", "sex", "current_points"} <= fields
    # 난이도 폐기(#428): activity_level 은 응답에서 제거됐다.
    assert "activity_level" not in fields
    # 기존 계정 필드도 유지된다(하위호환).
