...
private fun ProfileStep(vm: OnboardingViewModel) {
    ...
    // 제목은 외래어 없이 우리말로 쓴다(시니어 대상). 1·3단계가 모두 '건강'으로 시작해 구분이 약하던 것도 함께 정리.
    // 1단계 제목은 설정 '내 정보'의 같은 편집 카드와 **같은 말('기본 정보')로 맞춘다**(#391). 온보딩에서 넣은
    //   값을 나중에 설정에서 고치게 되는데, 두 화면이 다른 말을 쓰면 같은 것인 줄 모른다. '체격/신체'처럼 키·
    //   몸무게·허리둘레만 가리키는 말은 이 단계의 생년월일·성별을 덮지 못해 '기본 정보'로 간다.
    // 부제에 있던 "기본 정보를"은 제목과 같은 말이 되어 뺀다.
    val (title, subtitle) = when (subStep) {
        0 -> "기본 정보" to "맞춤 미션을 위해 몇 가지만 알려주세요."
