 ...
 *
 *  폰트: Pretendard(OFL, 상업/임베드 무료). 용량 최소화를 위해 가변폰트 한 파일을 번들하고
...
/** 앱 기본 글꼴(Pretendard). Theme 에서 bare Text 의 기본 글꼴로 provide 한다. */
val AigoFontFamily: FontFamily = Pretendard

// 문서 §2에서 크기를 지정한 7개 스타일은 아래에서 완전 재정의하고,
// 나머지 8개(display*/headlineMedium/headlineSmall/titleSmall/bodySmall/labelSmall)는
// M3 기본 메트릭을 유지하되 글꼴만 Pretendard 로 교체한다.
...
val Typography = Typography(
    displayLarge = Default.displayLarge.withPretendard(),
    displayMedium = Default.displayMedium.withPretendard(),
    displaySmall = Default.displaySmall.withPretendard(),
    // 큰 화면 타이틀
    headlineLarge = TextStyle(
        fontFamily = Pretendard, fontWeight = FontWeight.Bold,
        fontSize = 30.sp, lineHeight = 40.sp,
    ),
    headlineMedium = Default.headlineMedium.withPretendard(),
    headlineSmall = Default.headlineSmall.withPretendard(),
    // 화면 제목
    ...
    titleSmall = Default.titleSmall.withPretendard(),
    // 본문(기본)
    bodyLarge = TextStyle(
    ...
    bodySmall = Default.bodySmall.withPretendard(),
    // 버튼 글자
    ...
    labelSmall = Default.labelSmall.withPretendard(),
)
