...
class WalkingStepDetectorAccuracyTest {
    ...
    // ── 5. 간격-단독 한계 — 명시적 고정(에너지 대비 없는 희소 신호에서는 여전) ──────────

    @Test
    fun `간격 단독으로는 - 보행 직후 곧바로 앉기(대역 안 882ms)를 분리하지 못한다(에너지 정보 없는 희소 신호)`() {
        // 3차 실측(#89, 2026-07-22): 정상 20보 → 곧바로 앉기 5회 = 28.
        // 앉기 피크 간격 ≈882ms(68보/분)는 정상 보행 대역(500~1000ms) 한가운데 → interval <= max(1000) 통과.
        // 간격이라는 단일 신호로는 정상 보행과 분리 불가(docs §5-5).
        // ★ #131 보완(WalkStopGateLogic)은 '에너지'로 가른다. 이 테스트의 peak()은 walk·sit 모두 동일한
        //   2샘플 스파이크라 **에너지 대비가 없어** 게이트가 작동할 근거가 없다(간격만 있는 신호). 그래서
        //   이 케이스는 여전히 간격-단독 한계로 남는다. 실제/조밀 파형의 앉기 억제는
        //   WalkingStepDetectorStopGateTest(에너지 붕괴 감지) 가 검증한다.
        ...
        // 에너지 대비가 없는 희소 신호라 게이트가 뜨지 않고, 앉기 피크가 대역 안이라 계속 카운트 → 과다.
        assertTrue(
            "에너지 정보 없는 희소 신호에서는 간격-단독 분리가 불가해 과다카운트가 남아야 하는데 count=${logic.count} " +
                "(조밀/실제 파형 억제는 WalkingStepDetectorStopGateTest 참조)",
