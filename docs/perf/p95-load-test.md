...
배정 방식(스크립트 동작): 본 측정은 **실계정 시나리오(main_real, VU 수 = 토큰 수)와 게스트
시나리오(main_guest, VU 수 = VUS - 토큰 수)로 명시 분리**된다 — `VUS=50, TOKENS 2개`면 본 측정
내내 실계정 2 VU + 게스트 48 VU 가 보장된다. 실계정 시나리오 안에서는 시나리오-로컬
ID(`exec.vu.idInScenario`)로 토큰을 1:1 배정하므로, 전역 `__VU` 번호나 k6 의 시나리오 간 VU
재사용 방식에 의존하지 않는다. 전 VU 실계정은 VUS 수만큼 토큰을 넘기면 된다. 임계값 판정은
두 본 측정 시나리오 공통의 `phase:main` 태그 기준이라 워밍업만 제외된다.
